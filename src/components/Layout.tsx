  ...
  return (
    <div className="min-h-screen bg-[var(--bg-app)] flex flex-col font-sans">
      <div className="w-full flex-1 bg-[var(--bg-card)] shadow-2xl relative overflow-hidden flex flex-col transition-colors duration-300">
        ...
        {/* Status Bar */}
        <div className="h-8 sm:h-10 px-4 sm:px-8 flex justify-between items-center text-[10px] font-bold text-[var(--text-muted)] z-20">
        ...
        {/* Header Content */}
        <header className="relative z-20 px-4 sm:px-8 pt-2 sm:pt-4 pb-2">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 text-left">
                {!leftAction && !showBack && (
                  <div className="w-8 h-8 sm:w-10 sm:h-10 overflow-hidden rounded-xl bg-[var(--bg-button)] p-1 flex items-center justify-center shadow-[0_0_15px_rgba(37,99,235,0.3)] border border-blue-500/20">
                    <img src="/dragon_bg.png" alt="Logo" className="w-full h-full object-contain" />
                  </div>
                )}
                <h1 className="text-[20px] sm:text-[28px] font-black tracking-tight text-[var(--text-main)] uppercase italic leading-none" id="page-title">
                  {title}
                </h1>
              </div>
            </div>
          ...
          {subtitle && (
            <div className="text-[8px] sm:text-[10px] font-extrabold text-blue-500 uppercase tracking-[0.2em] mt-1">
              {subtitle}
            </div>
          )}
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-8 relative z-10 scrollbar-hide pb-20">
          <motion.div
